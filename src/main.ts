...
program
  .version("1.2.10", "-v, --version", "Output the version number")
  .name("lsdl")
...
const image_dl = async (img_url: string, png: string, gif?: string): Promise<void> => {
  const res: AxiosResponse<Buffer> = await axios.get(img_url, { responseType: 'arraybuffer' });
  fs.writeFile(png, Buffer.from(res.data), "binary").then((): void => {
